    ...
    companion object{
        fun random(): LootBox<Loot> = LootBox(
            contents = when (Random.nextInt(1..100)){
                in 1..5 -> Fez("фрезка безупречного стиля", 150)
                in 6..10 -> Fedora("федора знаний", 125)
                in 11..15 -> Fedora("потрясающая фетровая шляпа бирюзового цвета", 75)
                in 16..30 -> Fez("обычная феска", 15)
                in 31..50 -> Fedora("обычная феска", 10)
...
class Gemstones(
    ...
    override val name = "мешок драгоценных камней $value золота"
